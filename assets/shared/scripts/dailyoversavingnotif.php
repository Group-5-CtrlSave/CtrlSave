...
// Get all users with active budget rules
    ...
    while ($budgetVersionRow = mysqli_fetch_assoc($budgetVersionResult)) {
        ...
        if (mysqli_num_rows($allocationResult) > 0) {
            ...
            while ($allocationRow = mysqli_fetch_assoc($allocationResult)) {
                ...
                if ($limitType == 1) {
                    $budgetLimit = ($totalIncome * $value) / 100; // Percentage
                } else {
                    $budgetLimit = $value; // Fixed
                }

                // Get today's total saved
                ...
                if ($totalSaved > $budgetLimit) {
                    ...
                    // Prevent duplicate daily notifications
...
echo "Daily oversaving cron executed.";
?>
