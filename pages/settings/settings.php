...
<body>
    ...
    <div class="bg-green-custom"
        style="position: fixed; top: 72px; left: 0; width: 100%; height: calc(100vh - 72px); display: flex; flex-direction: column;">
            ...
            <div class="settings-container mt-4 mb-4 px-3">
                <div class="d-flex flex-column gap-3">
                    <?php foreach ($cards as $c): ?>
                        <div class="settings-card d-flex justify-content-between align-items-center px-4 py-3 rounded-3 <?= isset($c['disabled']) && $c['disabled'] ? 'disabled' : '' ?>"
                            ...
                            <?php if (isset($c['link'])): ?>
                                ...
                                <?php else: ?>
                                    <a href="<?= $c['link'] ?>" class="btn btn-sm fw-semibold px-4 py-2 bg-yellow-custom"
                            ...
                            <?php else: ?>
                                <button class="btn btn-sm fw-semibold px-4 py-2 bg-yellow-custom"
                                    style="flex-shrink: 0; white-space: nowrap;" onclick="openModal('<?= $c['modal'] ?>')">
                                    Edit
                                </button>
                            <?php endif; ?>
                        </div>
                ...
                <div class="needs-wants-table" style="max-height: 400px; overflow-y: auto;">
                    ...
                    <?php foreach ($expenseCategories as $cat): ?>
                        <div class="row align-items-center table-row">
                            ...
                            <div class="col-3 text-center">
                                <input type="radio" name="necessity[<?= $cat['userCategoryID'] ?>]" value="want"
                                    <?= $cat['userNecessityType'] === 'want' ? 'checked' : '' ?>>
                            </div>
                        </div>
                    <?php endforeach; ?>
                </div>
    ...
    <!-- Flexibility Modal -->
    <div id="flexibilityModal" class="modalOverlay" onclick="overlayClose(event,'flexibility')"
        style="backdrop-filter: none !important; -webkit-backdrop-filter: none !important;">
        <div class="modalBox" onclick="event.stopPropagation()">
            ...
            <form method="POST" action="">
                <div class="needs-wants-table" style="max-height: 400px; overflow-y: auto;">
                    ...
                    <?php foreach ($flexCategories as $cat): ?>
                        <div class="row align-items-center table-row">
                            <div class="col-6 text-start fw-medium">
                                <?= htmlspecialchars($cat['categoryName']) ?>
                            </div>
                ...
                <button type="submit" name="updateFlexibility" class="btn w-100 fw-semibold bg-yellow-custom mt-3">
                    Save Changes
                </button>
            </form>
        </div>
