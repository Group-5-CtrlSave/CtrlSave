...
<head>
    ...
    <style>
        ...
        @media screen and (min-width:414px) {
            .scrollable-container { height: 370px; }
        }
        @media screen and (min-width:390px) {
            .scrollable-container { height: 300px; }
        }
        @media screen and (min-width:430px) {
            .scrollable-container { height: 330px; }
        }
    </style>
</head>
...
<body>
    ...
    <div id="trackingPromptModal" class="prompt-modal" style="display: <?php echo $show_tracking_prompt ? 'block' : 'none'; ?>;">
                    ...
                    <?php if (empty($categories)): ?>
                        <div class="row expensesTab">
                            <div class="col-12 text-center">
                                <p class="expenseName" style="color:black;">No expense categories are selected for budgeting.</p>
                            </div>
                        </div>
                    <?php else: ?>
                        ...
                        ?>
                            <div class="row expensesTab" data-id="<?= $id ?>">
                                <div class="col-4 expensesCol">
                                    ...
                                    <input type="hidden" name="<?= $name_base ?>_necessity" value="<?= htmlspecialchars($category['userNecessityType']) ?>">
                                </div>
                                ...
                                <div class="col-4 labelLimit">
                                    <input class="form-control form-control-sm amountForm limit-input" type="text" name="<?= $name_base ?>_value" placeholder="Amount" <?= $input_disabled ?> <?= $input_opacity ?>>
                                </div>
                                
                                <input type="hidden" name="<?= $name_base ?>_mode" value="<?= $category['userisFlexible'] == 0 ? 'track' : 'limit' ?>">
                            </div>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </div>
            ...
            <div class="container savingsContainer">
                ...
                <div class="row savingsTab">
                    ...
                    <div class="col-6 savingsLimit">
                        <input class="form-control form-control-sm savingsForm" type="text" name="savings_value" placeholder="Amount" required>
                    </div>
                </div>
    ...
    <script>
        ...
        // --- 1. Checkbox & Input Logic (Modified to use unique names and call validation) ---
        ...
        function confirmTracking() {
            // This function runs when the user clicks 'Continue' on the Tracking Only prompt.
            // It needs to gather all data from the main form and submit it through the trackingForm 
            // along with the confirmation flag.
            ...
            for (const [key, value] of formData.entries()) {
                // Ensure we don't accidentally add the checkboxes which might not be checked
                // We only need the mode (hidden input) and the value (text input).
